...
@section('content')
	...
	<input type="hidden" id="loadStatement" data-garages-id="{{ $garages->id }}"></input>
				...
				<div class="statement_bank">
					...
					<div class="print-download ml-auto">
						<!-- <a href="javascript:window.print()" class="btn btn-light border text-black-50 shadow-none"><i class="fa fa-print"></i> Print &amp;
																				Download</a> -->
						<a href="{{ route('garages.statement.preview', $garages->id) }}" target="_blank"
							class="btn btn-primary">Preview</a>
						<a href="{{ route('garages.statement.download', $garages->id) }}"
							...
							<div class="col-lg-5 col-md-5 col-sm-6 col-12 offset-md-2 text-sm-end">
								<address class="text-sm-end">
									...
									@elseif(!empty($garage->logo) && file_exists($themeLogoPath))
										<img id="logo"
											src="{{ asset('frontend/themes/' . $garage->theme . '/img/logo/' . $garage->logo) }}"
											title="Garage Name" alt="Logo" width="auto" height="70" /><br>
									@else

										<img id="logo" src="{{ asset('frontend/themes/theme/img/logo/logo.png') }}"
											title="Garage Name" alt="Logo" width="auto" height="70" /><br>
									@endif
								</address>
							</div>
						...
						<div class="row align-items-end">
							<div class="col-lg-5 col-md-5 col-sm-6 col-12">
								<address>
									...
									Telephone: {{ $garages->garage_contact_number }}<br>
									Email: {{ $garages->garage_email }}<br>
									Address: {{ $garages->garage_address }}
								</address>
							</div>
						...
						<tbody>
							<tr>
								...
								<td class="text-right">£<span
										id="discountPrice">{{ number_format($discountPrice, 2) }}</span></td>
				...
				<div class="table-responsive">
					<table class="table table-striped">
						...
						<tbody id="transactionsTable">
							...
							<tr>
								<td class="text-right" colspan="6"><strong>Balance Due</strong></td>
	...
	<div class="modal fade" id="sendStatementModal" tabindex="-1" aria-labelledby="sendStatementModalLabel"
		...
		<div class="modal-dialog modal-md">
			<form action="{{ route('garages.statement.email') }}" method="POST">
				@csrf
				<input type="hidden" name="garages_id" value="{{ $garages->id }}">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="mb-3">
							...
							<input type="email" name="email_to" value="{{ $garages->garages_email }}" class="form-control"
								required>
						</div>
						<div class="mb-3">
						...
						<div class="form-group">
							...
							<textarea id="email_body" name="email_body">{!! getStatementEmailBody($garages) !!}</textarea>
						</div>
					</div>
	...
	<script>
		$(document).ready(function () {
			...
			$('#downloadPDF').click(function () {
				window.location.href = "{{ route('AutoCare.garages.statement.pdf') }}";
			});

			// Send Email
			$('#sendEmail').click(function () {
				$.ajax({
					url: "{{ route('AutoCare.garages.statement.email') }}",
					type: "POST",
		...
		$(document).ready(function () {
			$('#custom_to').on('change', function () {
				let range = $('#range').val();
				let fromDate = $('#custom_from').val();
				let toDate = $('#custom_to').val();
			...
			// Optional: Show/hide custom date inputs when range is changed
					...
					response.transactionsHtml.forEach(transaction => {
						transactionsHtml += `
																			<tr>
																				...
																				<td class="text-right">£${transaction.paid_price}</td>
																				<td class="text-right">£${transaction.discountPrice}</td>
																				<td class="text-right">£${transaction.balance_price}</td>
																			</tr>
																		`;
					});
