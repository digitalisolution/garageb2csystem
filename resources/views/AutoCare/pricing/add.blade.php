...
@section('content')
    <div class="container-fluid">
        <div class="bg-white p-3">
            <h5>{{ isset($tyrePricing) ? 'Edit' : 'Add' }} Tyre Pricing</h5>
            <!-- Display JSON Errors -->
            @if($errors->any())
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="supplier_id" name="supplier_id" required>
                                ...
                                @foreach ($suppliers as $supplier)
                                                        <option value="{{ $supplier->id }}" {{ old('supplier_id', $tyrePricing->supplier_id ?? '') ==
        ...
        'selected' : '' }}>
                ...
                <ul class="nav nav-tabs mt-3" id="pricingTab" role="tablist">
                    <li class="nav-item">
                        <button class="nav-link {{ $activeTab == 'defaultmargin' ? 'active' : '' }}" id="simple-markup-tab"
                            data-bs-toggle="tab" data-bs-target="#simple-markup" type="button">Simple Markup</button>
                    </li>
                    <li class="nav-item">
                ...
                <div class="tab-content">
                    <div class="tab-pane fade {{ $activeTab == 'defaultmargin' ? 'show active' : '' }}" id="simple-markup">
                            <label for="default_price" class="form-label">Simple Price</label>

                            <div class="input-group">
                                <select class="form-control1" name="default_price_type" id="default_price_type">
                                    <option value="amount" {{ old('default_price_type', $tyrePricing->default_price_type ?? '') == 'amount' ? 'selected' : '' }}>
                                        Fixed Amount (£)
                                    </option>
                                    <option value="percentage" {{ old('default_price_type', $tyrePricing->default_price_type ?? '') == 'percentage' ? 'selected' : '' }}>
                                        Percentage (%)
                                    </option>
                                </select>
                                <input type="number" step="0.01" class="form-control" id="default_price"
                                    name="default_price" placeholder="Enter markup value"
                                    value="{{ old('default_price', $tyrePricing->default_price ?? '') }}">
                            </div>

                            <small class="text-muted">
                                Select whether markup should be a fixed amount or percentage.
                            </small>
                    </div>
                            ...
                            <tbody>
                                ...
                                @php
                                    if (is_array($priceData)) {
                                        $decodedPriceData = $priceData;
                                    } else {
                                        $decodedPriceData = json_decode($priceData ?? '{}', true) ?? [];
                                    }
                                    $sizesData = $decodedPriceData['sizes'] ?? [];
                                @endphp
                                ...
                                @foreach ($price_by_size as $size)
                                    <tr>
                                        ...
                                        @foreach ($categories as $categoryKey => $categoryName)
                                            <td>
                                                {{-- Retrieve the value from the decoded sizes data --}}
                                                @php
                                                ...
                                                <input type="number" step="0.01" class="form-control"
                                                    name="price_by_size[sizes][{{ $size }}][{{ $categoryKey }}]" value="{{ old("
                                                        price_by_size.sizes.$size.$categoryKey", $value) }}">
                                            </td>
                                        @endforeach
                                    </tr>
                                @endforeach
