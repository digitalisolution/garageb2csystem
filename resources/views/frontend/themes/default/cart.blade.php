    ...
    <table>
        ...
        <tbody>
            ...
            @foreach ($cartItems as $item)
                ...
                $imageUrl = $defaultTyreImage; // guarantee defined
                $onError  = '';
                ...
                <tr id="cart-item-{{ $item['product_id'] }}" data-id="cart-item-{{ $item['product_id'] }}"
                    ...
                    <td class="hidden-xs py-4 px-2"><img src="{{ $imageUrl }}" alt="{{ $item['model'] }}" class="cart_img" width="auto" height="auto" @if($item['type'] === 'tyre')
                             onerror="this.onerror=null;this.src='{{ asset('frontend/themes/default/img/product/sample-tyre.png') }}';" @endif></td>
