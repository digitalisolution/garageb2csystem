								...
								<tr>
									...
									<td>
										@if ($order->status === 'completed')
											<badge class="bg-success text-white rounded px-2 no-wrap">
												{{ strtoupper($order->status) }}</badge>
										@elseif ($order->status === 'cancelled')
											<badge class="bg-danger text-white rounded px-2 no-wrap">
												{{ strtoupper($order->status) }}</badge>
										@elseif ($order->status === 'processing')
											<badge class="bg-warning text-white rounded px-2 no-wrap">
												{{ strtoupper($order->status)}}</badge>
										@else
											<badge class="bg-primary text-white rounded px-2 no-wrap">
												{{strtoupper($order->status) }}</badge>
										@endif
									</td>
