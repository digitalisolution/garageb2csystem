...
class TyrePricingController extends Controller
{
    ...
    {
        $tyrePricings = TyrePricing::all();
        return view('AutoCare.pricing.manage', compact('tyrePricings'));
    }
    ...
    public function create()
    {
        ...
        return view('AutoCare.pricing.add', compact('suppliers', 'OrderTypes', 'categories', 'price_by_size'));
    }
    ...
    public function edit($id)
    {
        ...
        // dd($priceData);
        return view('AutoCare.pricing.add', compact('tyrePricing', 'suppliers', 'OrderTypes', 'categories', 'price_by_size', 'priceData'));
    }
    ...
    public function updateTyrePrices(array $data)
    {
        ...
        if ($supplierId != 0) {
            ...
            if ($supplier->id == 1) {
                ...
                // For other suppliers, only consider instock = 0
                $query->where('instock', '=', 0);
            }
        } else {
            // Exclude the "ownstock" supplier (id = 1) if no supplierId is provided
            $query->where('supplier_id', '!=', 1);
            $query->where('instock', '=', 0);  // For non-specified supplierId, instock should be 0
        }

        if ($orderTypeId != 0) {
            // Retrieve the order type from the 'order_types' table
            $orderType = DB::table('order_types')->find($orderTypeId);

            // If the order type is not found, return a 404 error
            if (!$orderType) {
                return response()->json(['error' => 'Order type not found'], 404);
            }

            // Get the name of the order type
            $orderTypeName = $orderType->ordertype_name;
        }

        // Retrieve products matching the conditions
        $products = $query->get();
        foreach ($products as $product) {
            ...
            // Determine the price columns to update
            if ($orderTypeId != 0) {
                // Handle specific order type price column update
                $priceColumn = null;
                switch (strtolower($orderTypeName)) {
                    ...
                    case 'trade':
                        ...
                        break;
                    default:
                        // If no matching order type is found, set the default price column
    ...
    public function processTyrePricingUpdate()
    {
        ...
        // Log::info('Tyre pricing processTyrePricingUpdate function successfully.');
    }
