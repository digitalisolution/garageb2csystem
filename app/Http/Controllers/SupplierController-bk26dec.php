...
class SupplierController extends Controller
{
    ...
    public function save(Request $request, $id = null)
    {
        try {
            ...
            /*$viewData['option1'] = 'Search Supplier';
            $viewData['optionValue1'] = "AutoCare/supplier/search";
            $viewData['option2'] = 'Add Supplier';
            $viewData['optionValue2'] = "AutoCare/supplier/add";*/
    ...
    // {
    //     try{
    //     $viewData['header_link'] = HeaderLink::where("menu_id", '6')->select("link_title", "link_name")->orderBy('id', 'desc')->get();
    //     if (($id != null) && (Supplier::where('id', $id)->forceDelete())) {
    //         $request->session()->flash('message.level', 'warning');
    //         $request->session()->flash('message.content', 'Supplier was Deleted !');
    //         $viewData['pageTitle'] = 'Supplier';
    ...
    //     }
    //     } catch (\Throwable $e) {
    //         \Log::error("Error trash supplier: " . $e->getMessage());
    //         return redirect()->back()->withInput()->with('error', $e->getMessage());
    //     }
    ...
    //     } catch (\Throwable $e) {
    //         \Log::error("Error fetching supplier trash list: " . $e->getMessage());
    //         return redirect()->back()->withInput()->with('error', $e->getMessage());
    //     }
    // }
    ...
    public function importTyres(Request $request)
    {
        ...
        public function importOwnstockTyres(Request $request)
        {
            ...
            foreach ($rows as $row) {
                ...
                // If brand name exists, fetch the corresponding brand_id from the tyre_brand table
                $manufacturerId = null;
                if ($brandName) {
                    ...
                    if ($brand) {
                        // Brand found, use the existing brand_id
            ...
            $seasonMap = [
                    ...
                    // $tyre_model = isset($row['Model']) ? preg_replace('/[^a-zA-Z0-9\s]/', ' ', $row['Model']) : null;
