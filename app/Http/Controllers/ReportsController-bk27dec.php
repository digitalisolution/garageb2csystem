...
class ReportsController extends Controller
{
        ...
        try {
            switch ($reportType) {
                case 'customer':
                    ...
                    break;
                case 'garage':
                    $data = $this->getGarageReports($dateRange);
                    break;
                case 'invoice':
    ...
    private function getCustomerReports($dateRange)
    {
       ...
       private function getGarageReports($dateRange)
{
    return Garage::leftJoin('invoices', function ($join) use ($dateRange) {
        ...
        ->select(
            ...
            DB::raw("DATE(garages.created_at) as created_date"),
            DB::raw('COUNT(invoices.id) as total_invoices'),
            DB::raw('ROUND(COALESCE(SUM(invoices.grandTotal), 0), 2) as total_invoice_amt'),
            DB::raw('ROUND(COALESCE(SUM(invoices.balance_price), 0), 2) as total_due_amt')
        )
