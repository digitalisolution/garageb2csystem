  ...
  public function search(Request $request)
{
    ...
    $query = \App\Models\TyresProduct::from("$tyresTable as tyres_product")
        ...
        ->leftJoin('suppliers', 'tyres_product.supplier_id', '=', 'suppliers.id')
        ->leftJoin('garages', 'suppliers.garage_id', '=', 'garages.id')
        ->select('tyres_product.*', 'tyre_brands.name as brand_name','garages.garage_name');
    ...
    // Apply filters
    if ($request->filled('tyre_supplier_name')) {
        // Split selected value into supplier and garage if needed
        ...
        if ($garageName) {
    $query->where('garages.garage_name', $garageName);
}
    ...
    // Fetch distinct suppliers with garage names
    $suppliersWithGarage = \App\Models\TyresProduct::from($tyresTable)
        ->leftJoin('garages', 'tyres_product.garage_id', '=', 'garages.id')
        ->select('tyres_product.tyre_supplier_name', 'garages.garage_name')
        ->distinct()
        ->get()
        ->map(function ($item) {
            $item->display_name = $item->tyre_supplier_name
                . (!empty($item->garage_name) ? ' - ' . $item->garage_name : '');
            return $item;
        });
    ...
    $filters = $request->all();
        ...
        return view('AutoCare.tyres.search', array_merge($viewData, compact('tyres', 'brands', 'tyreTypes', 'sortBy', 'order', 'seasonTypes', 'vehicleTypes', 'suppliersWithGarage', 'suppliers', 'filters', 'totalQty', 'totalCost')));
}
