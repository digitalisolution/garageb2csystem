...
use HasPermissionCheck;
    ...
    {
        $this->authorizePermission('supplier.delete');
        try{
    ...
    public function importOwnstockTyres(Request $request)
    {
        ...
        //$supplierId = $request->input('supplier_id', 1);
        $supplierName = $request->input('supplier_name', 'ownstock');
        $supplier = Supplier::where('supplier_name', 'ownstock')->where('garage_id', $garageId)->first();
        if (!$supplier) {
        ...
        if ($request->has('delete_existing') && $request->delete_existing == 1) {
            ...
            TyresProduct::where('supplier_id', $supplierId)->where('tyre_supplier_name', $supplierName)->delete();
